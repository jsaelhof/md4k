...
export const thirdPartyBackdrop = async ({ imdbID }, _, { dataSources }) => {
  ...
  const { backdrop_path } = await dataSources.TMDB.getMovie(findResults[0].id);

  return toTMDBImageUrl(backdrop_path);
};
